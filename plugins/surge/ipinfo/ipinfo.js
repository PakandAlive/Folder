...
async function getProxyRequestInfo({ PROXIES = [] } = {}) {
  const { PROXY_IP, PROXY_INFO, PROXY_PRIVACY } = await getProxyInfo(undefined, $.lodash_get(arg, 'LANDING_IPv4') || 'gooodjob')
  let result
  if ($.isSurge() || $.isStash()) {
...
async function getProxyInfo(ip, provider) {
  ...
  const msg = `使用 ${provider || 'gooodjob'} 查询 ${ip ? ip : '分流'} 信息`

  if (provider == 'gooodjob') {
    try {
      const res = await http({
        ...(ip ? {} : getNodeOpt()),
        url: `https://info.gooodjob.me${ip ? `/${encodeURIComponent(ip)}` : ''}`,
        headers: {
      ...
      PROXY_INFO = [
        [
          '位置:',
          $.lodash_get(body, 'emoji_flag'),
          $.lodash_get(body, 'country_name'),
          $.lodash_get(body, 'region'),
          $.lodash_get(body, 'city'),
        ]
          .filter(i => i)
          .join(' '),
        ['经纬度:', `${$.lodash_get(body, 'latitude')}, ${$.lodash_get(body, 'longitude')}`].filter(i => i).join(' '),
        ['ASN:', $.lodash_get(body, 'asn.asn') || '-'].filter(i => i).join(' '),
        ['所属公司:', $.lodash_get(body, 'company.name') || '-'].filter(i => i).join(' '),
        ['信任分数:', $.lodash_get(body, 'threat.scores.trust_score') || '-'].filter(i => i).join(' '),
        ['VPN分数:', $.lodash_get(body, 'threat.scores.vpn_score') || '-'].filter(i => i).join(' '),
        ['代理分数:', $.lodash_get(body, 'threat.scores.proxy_score') || '-'].filter(i => i).join(' '),
        ['威胁分数:', $.lodash_get(body, 'threat.scores.threat_score') || '-'].filter(i => i).join(' ')
      ]
        .filter(i => i)
        .join('\n')
    } catch (e) {
