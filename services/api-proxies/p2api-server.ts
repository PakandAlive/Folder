...
async function proxyOpenAIChatCompletion(ctx: Context, requestBody: any, upstreamModel: string) {
  ...
  // 只在传入max_tokens时才添加该参数
  if (max_tokens && typeof max_tokens === 'number' && max_tokens > 0) {
    payload.max_tokens = max_tokens;
  }
  
  // 不传递temperature、top_p等参数，让上游API使用默认值
  // 因为Pollinations API对这些参数有严格限制，传入非默认值会导致500错误
  console.log(`Sending request to upstream: ${upstreamModel}, messages count: ${modifiedMessages.length}`);
